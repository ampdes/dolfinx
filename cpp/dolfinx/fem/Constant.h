...
public:
  ...
  /// @param[in] c Value of the constant
  explicit Constant(std::initializer_list<T> c)
  ...
  /// @brief Create a rank-1 (vector-valued) constant
  /// @param[in] c Value of the constant
  explicit Constant(std::span<const T> c) : Constant(c, {c.size()}) {}

  // /// @brief Create a rank-d constant.
  // /// @param[in] c Value of the Constant (row-major storage)
  // /// @param[in] shape Shape of the Constant
  // template <typename Values, typename Shape>
  //   requires(std::unsigned_integral<typename Shape::value_type>)
  // Constant(std::initializer_list<T> c, Shape shape)
